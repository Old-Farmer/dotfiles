...
function M.im_enter(mode, buf, async)
  ...
  if client ~= nil then
    ...
    elseif mode == "search" then
      data = "/"
    elseif mode == "cmdline" then
      data = "c"
    elseif mode == "terminal" then
      data = "t"
    end
    client:write(data, function() end)
    return
  end
...
function M.setup(user_opts)
  ...
  if (vim.env.SSH_CLIENT ~= nil or vim.env.SSH_TTY ~= nil) then
    if opts.ssh.enabled then
      client = require("imas.network").create_client(opts.ssh.host, opts.ssh.remote_port)
    else
      return
    end
  end
  ...
  if opts.ssh.enabled then
    vim.api.nvim_create_user_command("IMASCreateServer", function()
      Imas_server = require("imas.network").create_server(opts.ssh.host, opts.ssh.local_port)
    end, {})
  end
